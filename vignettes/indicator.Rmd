...
**Note: These instructions assume you have mapped `I:/SPD/Science Policy & Economics/State of Environment/_dev/` to your `D:/` drive**
...
1) In your bash shell, make new directory for the new indicator in 
  `D:/git_server/[file_name].git`, then create a *bare* git repo.  This will be the 
...
```{r}
setwd("c:/_dev/")
indicator_skeleton(path = ".", git_init = FALSE, 
                   git_clone = "D:/git_server/contaminants/tarballs.git")
```

This will clone the empty `tarballs.git` repository into a new folder called `tarballs`, 
add a default `.gitignore` file, start an RStudio project, and add the boilerplate 
indicator file/folder structure. For local git server folders you can ignore the LICENSE file by using apache = FALSE. You can then open the project from RStudio and you're off to the races!
...
```{r}
setwd("c:/_dev")
indicator_skeleton(path = "tarballs", git_init = FALSE)
```

## Creating a print version

Once you have a mostly final and functioniong html web page for the indicator, 
you can create the printable version:

```{r}
create_print_ver(html = "p:/indicators/sustainability/tarballs.html")
```

This will create a printable version template, converting the html to markdown,
and adding the Rmarkdown YAML header to the RMarkdown file.

# Git notes

## If you need to move an origin:

- You can check your origin by opening the shell in you local repository 
and using 'git remote -v'

- To change the prigin, open a shell and navigate to your local repository.  Make sure all your 
...
```
- Move the origin file to new location
- Then change the url of the origin (likely to our local git_server directory)

```
git remote set-url origin "D:/git_server/[indicator].git"
git push origin --all
```
